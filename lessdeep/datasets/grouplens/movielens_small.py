...
def download_data():
    cache_dir = default_dir('grouplens/movielens-small', 'cache')
    data_dir = default_dir('grouplens/movielens-small', 'data')

    extract_dir = os.path.join(data_dir, 'ext')
    if not os.path.exists(extract_dir):
        local_file = _download_dataset('ml-latest-small.zip',
                                       'ml-latest-small.zip.md5', cache_dir)
